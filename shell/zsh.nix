...
}:
{
  programs.zsh = {
    ...
    syntaxHighlighting = {
      enable = true;
      package = lib.mkIf (builtins.pathExists /usr/share/zsh/plugins/zsh-syntax-highlighting) (
        pkgs.runCommandLocal "system-zsh-syntax-highlighting" { } ''
          mkdir -p $out/share
    ...
    initContent = lib.mkMerge [
      (lib.mkBefore (
        if (with config.programs.starship; enable && enableZshIntegration) then
          "prompt off"
        else
          ...
          ''
            ...
            source "${
              if (builtins.pathExists /usr/share/zsh-theme-powerlevel10k) then
